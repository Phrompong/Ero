    ...
    <Container onClick={onClick} onBlurCapture={onBlur} style={{ width: '100%' }}>
      <Wrapper style={{ width: '100%' }}>
        <div className="input-select">
          <Input type={"text"} placeholder={'กรุณาเลือก'} value={selected ? selected.nameTH : ''} style={{ width: '100%' }} />
          {isOpen ? <OpenArrow /> : <CloseArrow />}
        </div>
        {
          options.length === 0 ? <>
            <WrapperOption isOpen={isOpen} isEmtry={true} style={{ width: '100%' }}>
...
export const DropdownSelect = ({ options, setSelected, selected, searchFrom, isOpen, onClick, onBlur }) => {
  ...
  return (
    <Container onClick={onClick} onBlurCapture={onBlur}>
      <Wrapper>
        {searchFrom && <Input value={filter} onChange={(e) => setFilter(e.target.value)} placeholder={'กรุณาเลือก'} />}
        {
          options.length === 0 ? <>
